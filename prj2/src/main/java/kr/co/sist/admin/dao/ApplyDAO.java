		...
		try {
			...
			con=db.getConn();
			 ...
			 StringBuilder selectApply = new StringBuilder();
		        ...
		        if (!userName.isEmpty()) {
		            selectApply.append("AND u.name = ? ");
		        }

		        selectApply.append("GROUP BY a.apply_num, ja.job_num, ja.name, ja.title, r.r_num, r.TITLE, u.name, r.CAREER, a.read_state, a.inspec_date, u.user_id ")
